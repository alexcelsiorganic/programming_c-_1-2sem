...
String reverse(String& a) {
    String reverse;
    delete[] reverse.string;
    reverse.string = new char[a.size + 1];
    reverse.size = a.size;
    for (int i = 0; i < a.size; i++) {
        reverse.string[i] = a.string[a.size - i - 1];
    }
    reverse.string[a.size + 1] = '\0';
    return reverse;
}
...
String& MakePalindrom(String& s) {
    ...
    cout << middle << endl;
    int count = 0;
    for (int i = s.size - 1; i > middle - 1; i--) {
        s.string[i] = s.string[count];
        count++;
    }
    return s;
}
string StrToS(String& a) {
    string res;
    res = a.string;
    return res;
}
